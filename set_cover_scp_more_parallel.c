...
//Test
void evaporate(Session * session, Solution * solution) {
    int i;
    for (i=0; i<session->numberOfSets; i++) 
        session->pheromone[i] = session->rho*session->pheromone[i];
}

void depositLocal(ThreadData * TD, Solution * solution, int bId, int eId) {
    int i,j; 
    for (i=0; i<TD->session->numberOfSets; i++) {
        for (j=bId; j<eId; j++) {
            if (solution[j].setsChoosed[i]) 
                TD->deltaValues[i] += Q/solution[j].weight;
        }
    }
}

void mergePheromone(Session * session, ThreadData * threadData, int numberOfThreads) {
    int i,j; 
    for (i=0; i<numberOfThreads; i++) {
        for (j=0; j<session->numberOfSets; j++) {
            session->pheromone[j] += threadData[i].deltaValues[j];
        }
    }
}
//Test
...
void * buildSolutions(void * threadData) {
    ...
    depositLocal(TD, TD->solutions, beginIndex, endIndex);
}
...
void initializeThreadData(Session * session, Solution * solutions, 
    ...
    for (i=0; i<numberOfThreads; i++) {
        ...
        threadData[i].solutions = solutions;
        threadData[i].deltaValues = (float *)malloc(session->numberOfSets*sizeof(float)); 
    }
}
...
int main(int argc, char *argv[]) {
    ...
    initializeThreadData(&session, solutions, threadData, numberOfThreads);
    ...
    for (i=0; i<cycles; i++) {
        for (j=0; j<numberOfThreads; j++) {
            memset(threadData[j].deltaValues, 0, session.numberOfSets*sizeof(float)); 
        ...
        evaporate(&session, solutions);
        mergePheromone(&session, threadData, numberOfThreads);
